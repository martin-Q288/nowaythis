...
const LOW_AWARENESS_BLOCK=/(serayah|joey bada\$\$|joey badass|annaLynne mccord|annalynne|danny cipriani|khadijah haqq|danielle olivera|miranda hope|kaleb winterburn)/i;
...
function purgeNew(before){
  ...
  if(keep.length!==window.data.length){
    window.data.splice(0,window.data.length,...keep);
    try{persist()}catch{}
    try{render()}catch{}
  }
}
function install(){
  ...
  const wrapped=async function(){
    const before=new Set((window.data||[]).map(x=>x?.id));
    const out=await base.apply(this,arguments);
    purgeNew(before);
    return out;
  };
  wrapped.__krCelebGate=true;
  window.discoverReadyTen=wrapped;
}
