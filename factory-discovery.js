...
function rank(r){return Number(r.score||27)*10-Math.min(48,hoursOld(r.publishedAt||r.pubDate||''))+Number(r.koreaAwareness||4)*3+Number(r.visualScore||4)+(r.curated?50:0)}
...
function makeReady(r){
  ...
  return normalize([{
    ...
    publisher:r.publisher||'',publishedAt:r.publishedAt||'',discoveredAt:Date.now(),discovery:r.curated?'curated':'manual',
    ...
    radarMeta:{scoreBreakdown:r.scoreBreakdown||null,thumbnailOptionsKr:r.thumbnailOptionsKr||[],thumbnailFinalKr:r.thumbnailFinalKr||'',photoCandidate:r.photoCandidate||'',photoSource:r.photoSource||'',layoutKr:r.layoutKr||'',ctaKr:r.ctaKr||'',factSources:r.factSources||[]}
  }])[0];
}
