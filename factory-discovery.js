...
const BM_CHANGE=/(최초|기록|번째|년 만|생일|결혼|약혼|열애|이별|복귀|변신|헤어|삭발|금발|공개|출연|캐스팅|우승|수상|팔로워|증가|은퇴|first|record|birthday|wedding|engag|dating|breakup|return|debut|wins?|award|retir|million|billion|%|\d)/i;
const BM_ARCHIVE=/(과거|예전|당시|재조명|재소환|다시 화제|몇 년 전|옛 영상|옛날 영상|과거 영상|과거 발언|old (video|clip|interview|comments?)|throwback|resurfac|years ago|from 20\d\d)/i;
...
function benchmarkPattern(r){
  ...
  if(BM_ARCHIVE.test(t)&&(BM_KNOWN.test(t)||BM_CHANGE.test(t)))return'ARCHIVE + CURRENT PEG';
...
function patternBonus(code){return({'RECOGNITION + CHANGE':15,'DANGER → PAYOFF':15,'WEIRD BUT TRUE':12,'WHAT + WHY WOW':10,'RESULT FIRST':10,'VISUAL FIRST':9,'ARCHIVE + CURRENT PEG':8,'COMPLETE FACT':6})[code]||0}
...
function ownLearningBonus(code){try{return Number(window.NWOwnLearning?.bonusForPattern?.(code))||0}catch{return 0}}
function rank(r){
  const code=benchmarkPattern(r),aw=koreaAwareness(r),visual=Math.max(1,Math.min(5,Number(r.visualScore||4))),own=ownLearningBonus(code);
  return Number(r.score||27)*8-Math.min(48,hoursOld(r.publishedAt||r.pubDate||''))*.8+aw*8+visual*6+patternBonus(code)+own+(r.curated?50:0);
}
...
function captionKr(r,title){const pub=r.publisher?` 보도 출처는 ${r.publisher}입니다.`:'';const archive=benchmarkPattern(r)==='ARCHIVE + CURRENT PEG'?'\n\n이 소재가 과거 영상·발언을 재조명한 경우, 당시 촬영/발언 시점과 지금 다시 화제가 된 이유를 반드시 분리해 표기하세요.':'';return `${title}\n\n현재 한국 뉴스·소셜에서 확인할 가치가 높은 해외 이슈로 발굴된 콘텐츠입니다.${pub}\n\n제목만 보고 단정하지 말고 원문에서 사건이 벌어진 시점, 장소, 당사자 발언과 실제 사진·영상의 맥락을 확인한 뒤 게시해 주세요. 첫 장에서는 가장 놀라운 ‘확인된 사실’ 하나를 강하게 보여주고, 캡션에서는 사진만으로 알 수 없는 배경을 보충하는 방식이 좋습니다.${archive}\n\n원문 확인 후 사실관계를 보강해 발행해 주세요.\n\n#NOWAYTHIS #해외토픽 #오늘의이슈 #바이럴`}
...
function makeReady(r){
  ...
  const facts=['원문 출처 확인','게시 날짜·사건 시점 확인','사진·영상이 실제 사건 장면인지 확인','핵심 사실 교차 확인'];
  if(code==='ARCHIVE + CURRENT PEG')facts.push('과거 촬영·발언 시점과 현재 재조명 시점을 분리 표기','현재 사건과 과거 장면의 연결 근거 확인');
  return normalize([{
...
async function discoverTen(){
  ...
  try{
    ...
    toast(items.length===10?'벤치마크 + 우리 계정 학습까지 반영해 10개를 READY에 추가했습니다':`신규 후보 ${items.length}개를 READY에 추가했습니다`);
...
window.NWDiscoveryBenchmark={benchmarkPattern,patternBonus,koreaAwareness,rank,ownLearningBonus};
