...
const CHANGE=/(최초|기록|번째|년 만|년째|생일|결혼|약혼|열애|이별|복귀|변신|헤어|삭발|금발|공개|출연|캐스팅|우승|수상|팔로워|증가|first|record|birthday|wedding|engag|dating|breakup|returns?|debut|wins?|award|million|billion|%|\d)/i;
...
function infer(x){
  const t=txt(x), override=x?.benchmarkPatternOverride;
...
function panel(){
  ...
  return `<section class="benchmark-advisor" data-benchmark-advisor="1">
    ...
    <div class="bm-foot"><span>시각 신호 ${visual}/5</span><span>500채널 벤치마크 학습 규칙</span><span>출처·권리 확인은 별도 Hard Gate</span></div>
  </section>`;
}
function mount(){
  if(window.getContentFormat&&window.getContentFormat()==='carousel'){};
